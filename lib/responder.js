...
function responseDispatcher(response) {
  ...
  var responseTypeLen  = (responseType == null) ? 0 : responseType.length;
...
JSONSeqDispatcher.prototype.chunkedStream = function dispatchJSONSeqChunkedStream(
  contentType, response
  ) {
  this.stream('chunked', response);
};
